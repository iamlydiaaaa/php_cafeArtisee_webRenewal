...
for ($i=0; $i<count($_POST['chk']); $i++)
{
    ...
    else if ($_POST['act_button'] == "선택삭제")
    {
		...
		while ($row = sql_fetch_array($result))
		{
			...
			{
				// 원글 포인트 삭제
				if (!delete_point($row['mb_id'], $csconfig['bo_table'], $row['wr_id'], '쓰기'))
					insert_point($row['mb_id'], $board['bo_write_point'] * (-1), "{$board['bo_subject']} {$row['wr_id']} 글 삭제");

				// 업로드된 파일이 있다면
				$sql2 = " select * from {$g5['board_file_table']} where bo_table = '{$csconfig['bo_table']}' and wr_id = '{$row['wr_id']}' ";
				$result2 = sql_query($sql2);
				while ($row2 = sql_fetch_array($result2)) {
					// 파일삭제
					@unlink(G5_DATA_PATH.'/file/'.$csconfig['bo_table'].'/'.$row2['bf_file']);

					// 썸네일삭제
					if(preg_match("/\.({$config['cf_image_extension']})$/i", $row2['bf_file'])) {
						delete_board_thumbnail($csconfig['bo_table'], $row2['bf_file']);
					}
				}
				...
				// 파일테이블 행 삭제
				sql_query(" delete from {$g5['board_file_table']} where bo_table = '{$csconfig['bo_table']}' and wr_id = '{$row['wr_id']}' ");
			...
			{
				// 코멘트 포인트 삭제
				if (!delete_point($row['mb_id'], $csconfig['bo_table'], $row['wr_id'], '댓글'))
		...
		// 최근게시물 삭제
		sql_query(" delete from {$g5['board_new_table']} where bo_table = '{$csconfig['bo_table']}' and wr_parent = '{$write['wr_id']}' ");

		// 스크랩 삭제
		sql_query(" delete from {$g5['scrap_table']} where bo_table = '{$csconfig['bo_table']}' and wr_id = '{$write['wr_id']}' ");
...
if ($count_write > 0 || $count_comment > 0)
    sql_query(" update {$g5['board_table']} set bo_count_write = bo_count_write - '$count_write', bo_count_comment = bo_count_comment - '$count_comment' where bo_table = '{$csconfig['bo_table']}' ");
